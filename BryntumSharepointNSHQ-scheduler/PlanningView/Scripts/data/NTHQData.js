...
import { Store, EventStore, ResourceStore, DateHelper } from "../../node_modules/@bryntum/scheduler/scheduler.module.js";
...
export default class NTHQData {
    ...
    setTimespan (start, end) {
        ...
        me.loadData(me.filters).then(() => {
            ...
            if (scheduler) {
                ...
                if (me.scheduler.isScheduler) {
                    // When timespan is lower than 8 weeks fit all in one window
                    scheduler.timeAxisViewModel.forceFit = DateHelper.diff(start, end, 'day') <= 60;
    ...
    filter (filters = {}) {
        ...
        const me = this;
        const unplannedChanged = !!me.filters.unplanned !== !!filters.unplanned;
        ...
        // Add the person records when unplanned is included.
        if (unplannedChanged) {
            me.setNewData();
        }
        ...
        if (filters.eventTypes && filters.directorates) {
           ...
           me.timeRangesStore.filterBy(filterFn);

            if (me.scheduler.strategic) {
                store.group('StrategicApproachTypeLOEName');
            } else {
                store.clearGroupers();
            }
        }
    ...
    setNewData () {
        ...
        me.resourceStore.data = me.getResourceRecords(me.filters.unplanned);//raw data

        me.eventStore.data = me.getEventRecords();//event models

        me.eventStore.forEach(model => {
           model.resourceId = model.data.resourceId
        });

        me.scheduler.scheduler.project.timeRanges = me.getNonWorkingTimeRecords();
        me.sorter && me.sorter();
    }
    ...
    extractResourcesFromEvents() {
        ...
        events.forEach(event => {
            ...
            if (eventType && event.DirectorateOrDivision.length > 0) {
                ...
                if (eventType.NonWorking === true) {
                    event.style = 'background-color: ' + event.eventColor + ';';
                    event.setDuration(event.Duration, 'ms');
                    me.nonWorkingEvents.push(event);
                } else {
                        ...
                        case 'pts':
                            ...
                            if (personDetails) {
                                ...
                                if (person) {
                                    ...
                                    const endOfTourDate = DateHelper.clearTime(person.EndOfTour);
                                    const today = DateHelper.clearTime(new Date());

                                    if (endOfTourDate >= today) {
                    ...
                    if (!resources[resourceId] && resource) {
                        ...
                        resource.EventTagsId = event.EventTagsId;
        ...
        if (!me.scheduler.isCalendar) {
            me.personnelStore.forEach(record => {
                ...
                const endOfTourDate = DateHelper.clearTime(record.EndOfTour);
                const today = DateHelper.clearTime(new Date());
