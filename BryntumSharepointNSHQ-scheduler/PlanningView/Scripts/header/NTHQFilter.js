import { WidgetHelper, Popup, TabPanel, Checkbox } from "../../node_modules/@bryntum/scheduler/scheduler.module.js"
...
export default class NTHQFilter {
    ...
    getContainers (items, tabIndex, options = {}) {
        ...
        // Add include all personnel in first in container
        if (options.unplanned) {
            me.unplanned = new Checkbox({
                checked : false,
                cls     : 'nthqcategoryitem',
                hidden  : me.scheduler.isCalendar,
                text    : '<b>Include unplanned personnel</b>',
                tabIndex: tabIndex,
                typeId  : 'unplannedpersonnel'
            });

            containers[0].items.push(me.unplanned);
        }
    ...
    createPopup () {
        ...
        tabNames = tabNames.concat([
            { name: 'Applications', items: applications, options : { unplanned: true }},
    ...
    submitFilter () {
        ...
        if (me.unplanned.checked){
            filters.unplanned = true;
        }
    ...
    setFilters () {
        ...
        if (filters.unplanned) {
            me.unplanned.checked = true;
        }

        if (filters.unaligned) {
            me.unaligned.checked = true;
        }
