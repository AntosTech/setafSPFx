...
StartTest(t => {
  ...
  const notUsedList = t.getConfigParam('notUsedList') || [],
    commonList = [/DateHelper.unitAbbreviations\.\d\.\d/, /DateHelper.unitNames\.\d/, /DateHelper.nonWorkingDays\.\d/, /DateHelper.weekends\.\d/, /.*\.width$/, /.*\.height$/, /.*\.labelWidth$/, /.*\.editorWidth$/, 'PresetManager.secondAndMinute', 'PresetManager.minuteAndHour', 'PresetManager.hourAndDay', 'PresetManager.dayAndWeek'].concat(t.getConfigParam('ignoreList') || []),
  ...
  let locale, localeName, missing, redundant;
