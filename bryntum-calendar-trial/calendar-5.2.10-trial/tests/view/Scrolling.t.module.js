import { ProjectModel } from '../../build/calendar.module.js';
...
StartTest(t => {
    ...
    t.it('Should scroll to the visibleStartTime hour at first render', async t => {
        ...
        await t.waitFor(() => t.samePx(cell7AMRect.top + cell7AMRect.height, t.rect('.b-weekview .b-dayview-allday-row-start').bottom, 5));
    });
    ...
    t.it('Should scroll to the visibleStartTime hour when data is delayed', async t => {
        ...
        // This should just wait until after the events arrive, and make the day content
        // clientHeight smaller and the view gets one more go at scrolling to its start time.
        await t.waitFor(() => {
            const
                cell7AM = t.query('.b-weekview .b-dayview-timeaxis-time:contains(7)')[0],
                cell7AMRect = t.rect(cell7AM);

            return t.samePx(cell7AMRect.top + cell7AMRect.height,  t.rect('.b-weekview .b-dayview-allday-row-start').bottom, 5);
        });
    });
    ...
    t.it('Should support scrolling to a date', async t => {
        ...
        t.is(weekView.getPositionFromTime(1), 70, 'getPositionFromTime 1');
        ...
        t.isApprox(cell11AMRect.top + cell11AMRect.height, t.rect('.b-weekview .b-dayview-allday-row-start').bottom, 7, '11am scrolled into view');
    });
    ...
    t.it('Should support scrolling to an hour number', async t => {
        ...
        t.isApprox(cell10AMRect.top + cell10AMRect.height, t.rect('.b-weekview .b-dayview-allday-row-start').bottom, 6, '10am scrolled into view');
    });
