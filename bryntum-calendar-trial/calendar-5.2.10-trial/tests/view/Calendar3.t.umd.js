...
StartTest(t => {
  ...
  t.it('Must render events which start before an initial, hidden non working day', async t => {
    ...
    calendar.viewContainer.layout.setActiveItem(calendar.modes.day, undefined, {
      animation: false
    });
    ...
    calendar.viewContainer.layout.setActiveItem(calendar.modes.week, undefined, {
      animation: false
    });
    t.is(calendar.activeView.firstVisibleDate, new Date(2021, 11, 27));
    t.is(calendar.activeView.lastVisibleDate, new Date(2021, 11, 31));
    calendar.viewContainer.layout.setActiveItem(calendar.modes.month, undefined, {
      animation: false
    });
    t.is(calendar.activeView.firstVisibleDate, new Date(2021, 10, 29));
    t.is(calendar.activeView.lastVisibleDate, new Date(2022, 0, 7));
    calendar.viewContainer.layout.setActiveItem(calendar.modes.year, undefined, {
      animation: false
    });
