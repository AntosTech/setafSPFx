import { EventStore, ResourceStore, DomHelper, DateHelper, Rectangle, ArrayHelper } from '../../build/calendar.module.js';

StartTest(t => {
    let calendar, eventStore, resourceStore, harness, month, week;
    ...
    t.it('allDay event', async t => {
        ...
        t.chain(
            ...
            { click : '.b-monthview-content .b-calendar-cell[data-date="2020-01-04"]', offset : ['50%', '80%'] },
            ...
            // The allDayEvents animates to its height.
            {
    ...
    t.it('Many all day events in the all day row', async t => {
        ...
        await dayView.heightAnimation;

        // Scrollable overflow in expanded mode.
    ...
    t.it('All day row autoHeight', async t => {
        ...
        // Scrollable overflow in autoHeight mode.
        await t.waitFor(() => bodyElement.style.overflowY === 'auto');
    ...
    t.it('All day row autoHeight no overflow -> overflow', async t => {
        ...
        await t.waitForAnimations();
        await t.waitFor(() => !scrollable.hasOverflow() && calendar.element.querySelectorAll('.b-cal-event-wrap.b-allday').length === 4);

        // Non scrollable when there's no overflow.
        await t.waitFor(() => bodyElement.style.overflowY === scrollBarWidth ? 'hidden' : 'auto');

        // Widths must be synced.
        t.isApprox(Rectangle.client(dayContainerElement).width, Rectangle.client(cellContainer).width);

        // Increase to 45 events. The all day row must only grow up to 50% height and overflow
        eventStore.clearFilters();

        await t.waitFor(() => calendar.element.querySelectorAll('.b-cal-event-wrap.b-allday').length === eventStore.count);
        await t.waitForAnimations();

        // All day row must not exceed max-height : 50% at default settings
        t.isLessOrEqual(dayView.allDayEvents.height, dayView.height / 2);
