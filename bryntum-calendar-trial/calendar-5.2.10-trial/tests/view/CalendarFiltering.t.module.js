import { EventStore, ResourceStore } from '../../build/calendar.module.js';
...
StartTest(t => {
    ...
    t.it('Resource filtering', async t => {
        ...
        t.chain(
...

            {
                ...
                trigger      : { click : '[data-ref="resourceFilter"] .b-list-item.b-selected:contains(Resource 1)' }
            },

            { waitFor : () => t.query('.b-cal-event-wrap').length === 1, desc : 'Event 1 filtered out' },
            ...
            {
                waitForEvent : [calendar.modes.month, 'refresh'],
                trigger      : { click : '[data-ref="resourceFilter"] .b-list-item.b-selected:contains(Resource 2)' }
            },

            { waitFor : () => t.query('.b-cal-event-wrap').length === 0, desc : 'Both filtered out' }
        );
    });
    ...
    t.it('Should filter multi assigned task', async t => {
        ...
        t.chain(
            ...
            { waitForSelector : '.b-cal-event-wrap[data-event-id="1"]', desc : 'Event 1 visible after filter for Resource 2' },
            { click : '.b-list-item[data-id="r1"]' },
            { click : '.b-list-item[data-id="r2"]' },
            { waitForSelector : '.b-cal-event-wrap[data-event-id="1"]', desc : 'Event 1 visible after filter for Resource 1' },
            { click : '.b-list-item[data-id="r1"]' },
            { waitForSelectorNotFound : '.b-cal-event-wrap[data-event-id="1"]', desc : 'Event 1 is not visible after uncheck Resource 1 and Resource 2' }
        );
    });
