...
StartTest(t => {
  ...
  t.it('modeDefaults not reapplying initial defaults', async t => {
    ...
    t.isDeeply(t.query('.b-weekview .b-cal-cell-header:visible').map(d => d.offsetWidth), [200, 200, 200, 200, 200]);
  });
  ...
  t.it('should align overflow popup correctly in a scrolled page', async t => {
    ...
    calendar = await getCalendar({
      width: DomHelper.scrollBarWidth ? '100%' : 'calc(100% - 15px)',
      // Keep width standard regardless of platform scrollbars
      date: new Date(2019, 9, 14),
      sidebar: false,
      eventStore,
      resourceStore,
  ...
  t.it('Drilling down', async t => {
    ...
    t.chain({
      ...
      moveCursorTo: '[data-ref="prevButton"]',
      desc: 'Mouse over Previous button'
    }, {
      waitForElementVisible: '.b-tooltip :contains(Previous week)'
    }, {
      moveCursorTo: '[data-ref="nextButton"]',
      desc: 'Mouse over Next button'
    }, {
    ...
    // date ranges that would be *incorrect* if we saw them.
    {
      ...
      click: '.b-yearview-month-name:contains(October)',
      offset: [30, '50%']
    }, {
      waitFor: () => calendar.mode === 'month'
    }, {
    ...
    // Single click on a year cell should show the overflow popup
    {
      ...
      trigger: {
        click: '.b-yearview-month .b-calendar-cell[data-date="2019-10-17"]'
      }
    },
  ...
  t.it('Should refresh sparsely', async t => {
    ...
    t.chain({
      ...
      trigger: {
        click: '[data-ref="resourceFilter"] .b-list-item.b-selected'
      }
    }, next => {
  ...
  t.it('Events with no startDate should be omitted with no errors', async t => {
    ...
    await t.waitFor(() => {
      ...
      return (_calendar$activeView = calendar.activeView) === null || _calendar$activeView === void 0 ? void 0 : _calendar$activeView.refreshCount;
    });
    t.pass('Test passed');
  });
