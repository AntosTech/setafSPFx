...
StartTest(t => {
  ...
  t.it('With inline data', async t => {
    ...
    const resourceView = calendar.activeView,
      ...
      {
        hourHeight
      } = calendar.modes.weekResources.items[0];
    ...
    await t.dragBy(`#${matsView.id} .b-dayview-day-detail.b-calendar-cell[data-date="2019-10-16"]`, [16, hourHeight * 2], null, null, null, null, [16, hourHeight]);
    ...
    await t.doubleClick(`#${matsView.id} .b-dayview-day-detail.b-calendar-cell[data-date="2019-10-16"]`, null, null, null, [16, hourHeight]);
  ...
  t.it('Calendar resource view should have one focused vew', async t => {
    ...
    await t.waitForProjectReady(calendar);
    const views = [...calendar.activeView.viewCache],
      countFocused = () => views.filter(view => view.containsFocus).length;
    await t.click('[data-view-resource-id="bryntum"] .b-dayview-day-content .b-calendar-cell');
    t.is(countFocused(), 1, 'One focused view only');
    t.ok(views[0].containsFocus, 'Bryntum view is focused');
    await t.click('[data-view-resource-id="hotel"] .b-dayview-day-content .b-calendar-cell');
    t.is(countFocused(), 1, 'One focused view only');
    t.ok(views[1].containsFocus, 'Hotel view is focused');
    await t.click('[data-view-resource-id="mats"] .b-dayview-day-content .b-calendar-cell');
    t.is(countFocused(), 1, 'One focused view only');
