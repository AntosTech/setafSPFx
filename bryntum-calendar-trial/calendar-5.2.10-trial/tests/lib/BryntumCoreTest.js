...
Class('BryntumCoreTest', {
    ...
    override : {
        ...
        genericMouseAction : async function() {
            const
                { SUPER } = this,
                options   = arguments[3];
            ...
            await SUPER.call(this, ...arguments);
    ...
    methods : {
            ...
            if (!me.disableWaitingForCSSAnimations && !disableWaitingForCSSAnimations) {
                ...
                const hasNoAnimations = () => {
                    ...
                    return !me.global.document.getAnimations?.().some(a => {
                        ...
                        if (!el.matches(exemptAnimationTargets) && iterCount !== 'infinite' && !ignoreTransitionProperties[a.transitionProperty]) {
                            animatingEl = el;
                            return a;
                        }
                    });
        ...
        async forEveryThemeAndLocale(fn, args = []) {
            const
                { DomHelper } = this.global,
                defaultTheme  = DomHelper.getThemeInfo().name,
                locales       = Object.keys(this.global.bryntum.locales),
            ...
            for (const theme of themes) {
                ...
                for (const locale of locales) {
                    this.diag(`Test theme ${theme}, locale ${locale.name}`);
        ...
        smartMonkeys(description) {
            const
                ...
                targetSelectors = ['.demo-header', '.demo-toolbar', '.b-toolbar'].map(sel => `${sel} ${clickSelectors}`).join(','),
                elementsToClick = me.query(targetSelectors),
        ...
        async injectXSS(component, maxRecords = 5) {
                    ...
                    field.prototype.showPicker = function() {
                        ...
                        if (this.isPainted && !ignore.some(cls => this.element.classList.contains(cls)) &&
                            ...
                            const
                                ...
                                textWidth  = string ? DomHelper.measureText(string, this.input) : 0,
                                inputWidth = Rectangle.content(this.input).width;

                            if (inputWidth < textWidth) {
                                me.fail(`${field.type} input is overflowing : ref-${this.ref}, id-${this.id}, string: ${string}`);
                            }
                        }
                    };
                };
...
class BryntumTestHelper {
    ...
    static prepareMonkeyTests({ items, mode, examplesRoot = '../examples', config = {}, smartMonkeys = false, rtl = false, itemFilter = item => item }) {
        const
