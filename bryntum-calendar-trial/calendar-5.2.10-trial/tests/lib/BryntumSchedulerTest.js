    ...
    methods : {
        ...
        async getSchedulerAsync(config, nbrEvents) {
            ...
            if (scheduler.isVisible && config?.sanityCheck !== false) {
                await this.checkGridSanity(scheduler);
            }
        ...
        assertDependency(scheduler, dependency, { fromSide, toSide, fromBox, toBox } = {}, msg = 'Assert dependency') {
            ...
            if (from && to) {
                const
                    ...
                    dependencyPoints = dependencyEl.elementData.points,
                    depStartPoint    = dependencyPoints[0],
                    depEndPoint      = dependencyPoints[dependencyPoints.length - 1],
                    depFromPoint     = [depStartPoint.x1 + svgBox.left, depStartPoint.y1 + svgBox.top],
                    depToPoint       = [depEndPoint.x2 + svgBox.left, depEndPoint.y2 + svgBox.top],
                    gap              = 2;
        ...
        assertHorizontalBreakOnRowBorder(client, { dependencyId, assignmentData, rowId, expectedPoints = 6 }) {
            const
                ...
                depElement = client.getElementsForDependency(dependency, assignmentData?.from, assignmentData?.to)[0],
                path       = depElement.elementData.points,
                rowBox     = client.getRecordCoords(rowId, true),
                rowBottom  = rowBox.top + rowBox.height;

            if (expectedPoints >= 4) {
                this.ok(path.some(p => p.y1 === p.y2 && p.y1 === rowBottom), `Dependency ${dependency.id} is aligned with row boundary`);
            }

            this.is(path.length + 1, expectedPoints, `Points amount is correct for dependency ${dependency.id}`);
        },
         ...
         * @returns {boolean}
         */
        assertEventsPositions(scheduler, events = scheduler.events) {
            const
        ...
        assertExportedEventDependenciesList(doc, dependencies = []) {
            ...
            const
                ...
                getDepStartAndEndPoint   = el => {
                    const
                        instructions = el.getAttribute('d').split(' '),
                        // M300,4576 -> [300,4576]
                        start        = instructions[0].substring(1).split(',').map(p => parseFloat(p));
                    ...
                    while (instructions.length) {
                        const
                            // h50 | h-50 | v50 | v-50
                            step    = instructions.shift(),
                            // h | v
                            command = step.substring(0, 1),
                            // 50
                            len     = parseFloat(step.substring(1));
                ...
                getDependencyCoordinates = (dependency, dependencyEl, fromEl, toEl, scale) => {
                    const
                        ...
                        [
                            depStartPoint,
                            depEndPoint
                        ]            = getDepStartAndEndPoint(dependencyEl),
            ...
            dependencies.forEach(dep => {
                const depElement = doc.querySelector(`[data-dep-id="${dep.id}"]`);
...
BryntumSchedulerTest.prototype._getGridSync = BryntumGridTest.prototype.getGridSync;
