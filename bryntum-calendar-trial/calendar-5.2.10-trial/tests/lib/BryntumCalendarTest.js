Class('BryntumCalendarTest', {
    ...
    override : {
        action : async function(...actions) {
            ...
            // This also waits for project readiness
        ...
        checkCalendarSanity(widget) {
            const cells = widget.element.querySelectorAll('[data-date]');

            Array.from(cells).forEach(cell => {
                const date = this.global.DateHelper.parseKey(cell.dataset.date);

                if (!cell.classList.contains(`b-day-of-week-${date.getDay()}`)) {
                    this.fail(`b-day-of-week class not added to date ${cell.dataset.date} cell in ${widget.id}`);
                }
            });
        },
        ...
        async getMonthView(config) {
            ...
            monthView.getCellEvents = date => {
                ...
                return monthView.cellMap.get(date).renderedEvents.map(e => e.eventRecord);
            };
            monthView.getCellEventIds = date => {
                if (typeof date !== 'string') {
                    date = DateHelper.format(date, 'YYYY-MM-DD');
                }
                return monthView.cellMap.get(date).renderedEvents.map(e => e.eventRecord.id);
            };
            ...
            this.checkCalendarSanity(monthView);
        ...
        async getAgendaView(config) {
            ...
            this.checkCalendarSanity(agendaView);
        ...
        async getDayView(t, config) {
                    ...
                    dayView.allDayEvents?.on({
                        paint({ source }) {
                            const
                                ...
                                threshold          = t.bowser.firefox ? 1 : 0;
                            t.isLessOrEqual(contentElement.scrollHeight, contentElement.clientHeight + threshold, 'scrollHeight matches clientHeight height');
                        }
                    });
                },
                once : true
            ...
            this.checkCalendarSanity(dayView);
        ...
        async getYearView(config) {
            ...
            this.checkCalendarSanity(yearView);
        ...
        async getCalendar(config, activeIndex) {
            ...
            // We don't want any async waiting for fonts inside the tests
            ...
            this.checkCalendarSanity(calendar);
        ...
        async recurrenceEditConfirmation(el) {
            ...
            t.is(buttons.length, 3, 'Recurrence editor dialog');
        ...
        setupCalendarTest(harness, calendar) {
            ...
            if (calendar && calendar.element) {
                ...
                for (let i = 0, { length } = eventEls; i < length; i++) {
                    if (parseFloat(eventEls[i].style.top) > 100) {
                        this.fail(`Event ${eventEls[i].dataset.eventId} element is rendered out of bounds`);
                    }
                }
            }
            ...
            if (this.query('.b-overflowpopup,.b-sch-event-tooltip.b-eventeditor').length > 0) {
                this.selectorNotExists('.b-overflowpopup:visible');
                this.selectorNotExists('.b-sch-event-tooltip');
                this.selectorNotExists('.b-eventeditor');
            }
        },
        ...
        setupWeekViewAllDayDragHarness() {
            return this.setupDragHarness({
                activeIndex  : 1,
                viewSelector : ':is(.b-weekview, .b-calendarrow-overflowpopup)',
                cellCls      : '.b-calendar-cell.b-dayview-allday',
                config       : {
                    // defaults
                }
        ...
        setupDragHarness({ activeIndex, config, viewSelector, cellCls, innerSelector, overflowSuffix }) {
            ...
            const harness = {
                ...
                getDateRangeForElement(el, includeTop) {
                    const
                        view = t.global.bryntum.fromElement(el),
                        { rtl } = view,
                        cell = el.closest(`${view.visibleCellSelector}${view.hideNonWorkingDays ? ':not(.b-nonworking-day' : ''}`),
                        calendarRow = cell.closest('.b-calendarrow'),
                        calendarWeek = cell.closest('.b-calendar-week'),
                        cellDate = view.getDateFromElement(cell),
                    ...
                    if (cell.closest('.b-dayview-day-container')) {
                        ...
                        const toDate = x => {
                            const
                                ...
                                dx = rtl ? weekRect.right - x : x - weekRect.x;

                            d.setDate(d.getDate() + Math.round(dx * daysPerPx));

                            return d;
                        };

                        if (rtl) {
                            end = toDate(elRect.left);
                            start = toDate(elRect.right);
                        }
                        else {
                    ...
                    // Ensure narrow non breaking-space is converted to normal space for test purposes
                    const ret = [start, end, el.textContent.replaceAll('\u202f', ' ')];
