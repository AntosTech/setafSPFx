StartTest(async t => {
    ...
    let examples = Array.from(t.global.document.querySelectorAll('.example:not(.offline)'))
        .map(el => ({
    ...
    for (const example of examples) {
        t.describe(`Check example #${example.id}`, async t => {
            ...
            const closePopups = async() => {
                const selector = popups?.[example.id];
                if (selector) {
            ...
            t.it('Check trial button', async t => {
                if (!skipTrialCheck.find(skip => example.id.includes(skip))) {
                    ...
                    t.pass(`Trial button displayed for ${example.id}`);
                }
            });
            ...
            t.it('Check current config, should yield something', async t => {
                ...
                if (product?.isWidget) {
                    ...
                    if (!skipTestSizeCheck?.includes(example.name)) {
