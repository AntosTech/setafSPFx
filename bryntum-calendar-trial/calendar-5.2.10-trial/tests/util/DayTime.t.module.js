import { Model, DayTime, EventModel } from '../../build/calendar.module.js';

StartTest(t => {
    const
        MILLIS_MINUTE = 60 * 1000,
    ...
    function makeEvent(startDate, endDate) {
        return new EventModel({ startDate, endDate });
    }
    ...
    t.describe('Simple Shifted Day', t => {
        ...
        t.it('isInterDay', t => {
            // Crosses the day boundary which is 8am
            t.ok(dt.isInterDay(new Model({
                startDate  : makeDate('7:30'),
                endingDate : makeDate('8:30')
            })));
            // Doesn't cross the dya boundary - touches it
            t.notOk(dt.isInterDay(new Model({
                startDate  : makeDate('7:00'),
                endingDate : makeDate('8:00')
            })));
            // Doesn't cross the dya boundary - touches it
            t.notOk(dt.isInterDay(new Model({
                startDate  : makeDate('8:00'),
                endingDate : makeDate('9:00')
            })));
        });
    });
