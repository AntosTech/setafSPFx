import { DateHelper, DomHelper, DayTime, EventStore, ResourceStore } from '../../build/calendar.module.js';
...
StartTest(t => {
    ...
    function checkEventSanity(t, dayView) {
        ...
        // The "dates" in the collected day cells must be shifted in line with the view's startShift
        ...
        t.is(dayView.lastVisibleDate, endDate, 'Correct lastVisibleDate');
        ...
        events.forEach(event => {
            const
                eventEl    = dayView.getEventElement(event),
                { allDay } = event;

            if (eventEl) {
                t.isApproxPx(eventEl.offsetTop, allDay ? 0 : calcY(event.startDate), `Event ${event.name} top correct`);
                ...
                if (!dayView.dayEndTime) {
                    t.isApproxPx(
                        ...
                        allDay ? dayView.dayContainerElement.offsetHeight : DateHelper.as('hours', event.endDate - event.startDate, 'ms') * dayView.hourHeight,
    ...
    async function ready(t) {
        await t.waitForSelector('.b-dayview-timeaxis-time:first-child:regex(7\\sPM)');
    }
    ...
    function stairStepEvent(i, offset = 21, duration = 2) {
        ...
        return [
            i + Math.floor(start / 24), start % 24,
            i + Math.floor(end / 24), end % 24
        ];
    }

    t.beforeEach(() => {
        dayView = dayView?.destroy();
    ...
    t.it('sanity', async t => {
        ...
        await ready(t);
        await t.waitForSelector('.b-dayview-timeaxis-time:last-child:regex(5\\sPM)');
        ...
        t.isApproxPx(
            ...
            'DayView dayElement height is correct');

        dayView.dayStartTime = 20;
        // Wait for layout to be correct
        await t.waitForSelector('.b-dayview-timeaxis-time:first-child:regex(9\\sPM)');
        await t.waitForSelector('.b-dayview-timeaxis-time:last-child:regex(5\\sPM)');
        ...
        dayView.dayEndTime = 13;
        // Wait for layout to be correct
        await t.waitForSelector('.b-dayview-timeaxis-time:first-child:regex(9\\sPM)');
        await t.waitForSelector('.b-dayview-timeaxis-time:last-child :last-child:regex(1\\sPM)');
    ...
    t.it('Current time line', async t => {
        ...
        const
            indicator = document.querySelector('.b-current-time-indicator'),
    ...
    t.it('Interaction', async t => {
        eventStore = t.getEventStore({
            data : (function() {
                const events = [{
                    ...
                    name       : 'Assignment 10',
                    resourceId : 'r1',
                    startDate  : new Date(2011, 0, 6, 18),
                    endDate    : new Date(2011, 0, 7, 18)
                }];
        ...
        const event10 = eventStore.getById(10);

        t.is(event10.startDate, new Date(2011, 0, 6, 18));
        t.is(event10.endDate, new Date(2011, 0, 7, 18));

        t.selectorNotExists('[data-date="2011-01-05"] [data-event-id=10]');
        t.selectorExists('[data-date="2011-01-06"] [data-event-id=10]');
        t.selectorNotExists('[data-date="2011-01-07"] [data-event-id=10]');

        // Resize on left

        await t.moveMouseTo({
            target : '[data-event-id="10"]',
            offset : [5, '50%']
        });

        await t.dragBy({
            source   : '[data-event-id="10"] .b-gripper-vert',
            delta    : [-75, 0],
            dragOnly : true
        });

        await t.untilSelector('[data-date="2011-01-06"] .b-cal-tentative-event');

        let els = t.query('[data-date="2011-01-06"] .b-cal-tentative-event');
        let sizes = els.map(el => [el.style.left, el.style.width]);

        t.isDeeply(sizes, [['20%', '40%']]);

        await t.mouseUp();

        await t.waitForAnimationFrame();

        t.selectorNotExists('[data-date="2011-01-04"] [data-event-id=10]');
        t.selectorExists('[data-date="2011-01-05"] [data-event-id=10]');
        t.selectorExists('[data-date="2011-01-06"] [data-event-id=10]');
        t.selectorNotExists('[data-date="2011-01-07"] [data-event-id=10]');

        t.is(event10.startDate, new Date(2011, 0, 5, 18));
        t.is(event10.endDate, new Date(2011, 0, 7, 18));
        ...
        await t.untilSelector('[data-date="2011-01-07"] .b-cal-tentative-event');

        els = t.query('[data-date="2011-01-07"] .b-cal-tentative-event');
        sizes = els.map(el => [el.style.left, el.style.width]);

        t.isDeeply(sizes, [['20%', '60%']]);
        ...
        t.selectorExists('[data-date="2011-01-05"] [data-event-id=10]');
        t.selectorExists('[data-date="2011-01-06"] [data-event-id=10]');
        t.selectorExists('[data-date="2011-01-07"] [data-event-id=10]');
        t.selectorNotExists('[data-date="2011-01-08"] [data-event-id=10]');

        t.is(event10.startDate, new Date(2011, 0, 5, 18));
        t.is(event10.endDate, new Date(2011, 0, 8, 18));

        // Move event into allDay zone
        const event5 = eventStore.getById(5);
        const HR = dayView.hourHeight;

        await t.dragBy({
            source   : '[data-event-id="5"]',
            delta    : [0, -HR * 9 - 10],
            dragOnly : true
        });

        await t.untilSelector('.b-dayview-allday[data-date="2011-01-08"] .b-cal-tentative-event');

        els = t.query('.b-dayview-allday[data-date="2011-01-08"] .b-cal-tentative-event');
        sizes = els.map(el => [el.style.left, el.style.width]);

        t.isDeeply(sizes, [['80%', '20%']]);

        await t.mouseUp();

        t.selectorNotExists('[data-date="2011-01-07"] [data-event-id=5]');
        await t.waitForSelector('.b-dayview-allday[data-date="2011-01-08"] [data-event-id=5]');

        t.is(event5.startDate, new Date(2011, 0, 8, 18));
        t.is(event5.endDate, new Date(2011, 0, 9, 18));

        // Move event out of allDay zone
        await t.moveCursorTo('[data-event-id="10"]');
        ...
        await t.untilSelector('.b-dayview-day-detail[data-date="2011-01-08"] .b-cal-tentative-event');

        els = t.query('.b-dayview-day-detail[data-date="2011-01-08"] .b-cal-tentative-event');
        sizes = els.map(el => [el.style.top, el.style.height]);

        t.isDeeply(sizes, [['41.67%', '8.33%']]);

        await t.mouseUp();

        t.selectorNotExists('[data-date="2011-01-07"] [data-event-id=5]');
        t.selectorExists('[data-date="2011-01-08"] [data-event-id=5]:regex(11\\sPM)');

        t.is(event5.startDate, new Date(2011, 0, 8, 23));
        t.is(event5.endDate, new Date(2011, 0, 9, 0));

        // Get away from the resized event to avoid mouseovers
        await t.moveMouseTo({
            target : '.b-dayview-header',
            offset : [50, '50%']
        });
        ...
        await t.click('[data-event-id="1"] .b-cal-event');
    });
    ...
    t.it('Click interaction', async t => {
        ...
        await t.click('.b-dayview-day-content .b-calendar-cell[data-date="2019-10-15"]', null, null, null,
            ['50%', dayView.hourHeight * 3.5]);
    ...
    t.it('Adding new events', async t => {
        ...
        dayView.eventStore.add([
            { startDate : shifter(2019, 10, 14), endDate : shifter(2019, 10, 19), name : 'Work week' },
            { startDate : shifter(2019, 10, 19), endDate : shifter(2019, 10, 21), name : 'The weekend' },
            { startDate : shifter(2019, 10, 1), endDate : shifter(2019, 10, 31), name : 'Very long event' }
        ]);
    ...
    t.it('Events which end at EOD must not be in the all day region', async t => {
        ...
        t.selectorNotExists('.b-calendarrow .b-cal-event-wrap.b-allday:contains(Late dinner)',
            'Event is correctly not in all day zone');

        t.selectorCountIs('.b-dayview .b-dayview-day-container .b-cal-event-wrap:contains(Late dinner)', 1,
            'Event is in day detail zone');
    });
    ...
    t.it('Background lines must end at EOD', async t => {
        ...
        t.is(p, `polygon(0px 0px, 100% 0px, 100% ${dayHeight}px, 0px ${dayHeight}px)`,
            'Day container element clipped correctly');
    });
    ...
    t.it('Changing event startDate should work', async t => {
        ...
        let prom = t.waitForEvent(dayView, 'refresh');
        ...
        t.is(mondayCell.childNodes.length, 1, 'Monday has 1 event');
        ...
        t.is(tuesdayCell.childNodes.length, 2, 'Tuesday has 2 events');
        ...
        t.is(dayView.getEventElement(lunch), tuesdayCell.firstElementChild, 'Lunch element remains');
    ...
    t.it('should handle drag/drop', async t => {
        ...
        // https://github.com/bryntum/support/issues/3942
        ...
        await t.dragBy('.b-dayview-day-detail[data-date="2021-03-23"]', [2 * dayPx, 4 * hourPx], null, null, null, false,
            ['50%', 2 * hourPx]);
    ...
    t.it('should handle drag create', async t => {
        ...
        await t.dragBy('.b-dayview-day-detail[data-date="2021-03-23"]', [0, 6 * hourPx], null, null, null, true,
            ['50%', 2 * hourPx]);

        // This is not a day spanning event. It's within our logical day
        t.selectorCountIs('.b-dayview-day-detail[data-date="2021-03-23"] .b-cal-tentative-event', 1);
    });
    ...
    // https://github.com/bryntum/support/issues/5789
    t.it('should handle dblclick create', async t => {
        calendar = await t.getCalendar({
            date   : new Date(2021, 2, 27),
            events : [],
            modes  : {
        ...
        // Click on midnight. Bug was that this was taken as start of day and had
        // dayStartTime which is 18 in this example added to it. Must in fact start at 00:00
        await t.doubleClick('.b-dayview-day-detail[data-date="2021-03-23"]', null, null, null,
            ['50%', 6 * hourPx]);

        // This is not a day spanning event. It's within our logical day
        t.selectorCountIs('.b-dayview-day-detail[data-date="2021-03-23"] .b-iscreating', 1);

        await t.waitFor(() => calendar.features.eventEdit.editor?.containsFocus);

        await t.type(null, 'This is at midnight[ENTER]');

        // Start date is correct at midnight
        t.is(calendar.eventStore.first.startDate, new Date(2021, 2, 24));
    });
});
