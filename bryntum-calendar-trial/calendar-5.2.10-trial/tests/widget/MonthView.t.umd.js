...
StartTest(t => {
  ...
  t.beforeEach(t => {
    var _monthView, _monthView$destroy, _calendar;
    (_monthView = monthView) === null || _monthView === void 0 ? void 0 : (_monthView$destroy = _monthView.destroy) === null || _monthView$destroy === void 0 ? void 0 : _monthView$destroy.call(_monthView);
  ...
  t.it('sanity', async t => {
    ...
    t.is(Array.from(monthView.contentElement.querySelectorAll(`.${monthView.dayCellCls}`)).filter(isNotHideDisplay).length, 42);
    monthView.hideNonWorkingDays = true;
    t.is(Array.from(monthView.contentElement.querySelectorAll(`.${monthView.dayCellCls}`)).filter(isNotHideDisplay).length, 30);
  ...
  t.it('Interaction', async t => {
    ...
    ['weeknumbermousedown', 'weeknumbermouseup', 'weeknumberclick', 'daynumbermousedown', 'daynumbermouseup', 'daynumberclick', 'eventmouseover', 'eventmousedown', 'eventmouseup', 'eventclick'].forEach(eventName => t.firesOnce(monthView, eventName));
    t.chain({
      click: '.b-week-num'
    }, {
      click: '.b-day-num'
  ...
  t.it('Two day events', async t => {
    ...
    const e5 = monthView.eventStore.getAt(4),
    ...
    const allDayEventRule = CSSHelper.findRule('.b-solid-bar .b-cal-event'),
  ...
  t.it('Events overflowing a cell', async t => {
    ...
    {
      waitForEvent: [monthView, 'refresh'],
      trigger: () => eventStore.remove(eventStore.last)
    }, () => {
  ...
  t.it('dayCellRenderer should be able to affect cell style', async t => {
    const eventStore = t.getEventStore({
      ...
      colours = [['rgb(255, 0, 0)', 'red'], ['rgb(255, 165, 0)', 'orange'], ['rgb(255, 255, 0)', 'yellow'], ['rgb(0, 128, 0)', 'green'], ['rgb(0, 0, 255)', 'blue'], ['rgb(75, 0, 130)', 'violet'], ['rgb(238, 130, 238)', 'pink']];
    let setColour = true;
    monthView = await getMonthView({
      width: 1000,
      height: 750,
    ...
    for (let columnIndex = 0; columnIndex < 7; columnIndex++) {
      ...
      cells.forEach((cell, cellIndex) => {
        t.hasStyle(cell, 'backgroundColor', colours[columnIndex][0], 'background-color is ' + colours[columnIndex][1] + ' for ' + cellIndex + ' cell in ' + columnIndex + ' column');
  ...
  t.it('Week row sizing', async t => {
    ...
    let flexedElement, flexedWeekStart, shrinkWrappedElement, shrinkWrappedWeekStart;
    monthView = await getMonthView({
      width: 1000,
      // Start small because we are testing overflow
      height: 500,
    ...
    const weekStartDate = new Date(2011, 0, 9),
  ...
  t.it('eventHeight', async t => {
    ...
    const e1Element = monthView.getEventElement(eventStore.first).querySelector('.b-cal-event'),
  ...
  t.it('Should switch to month view upon click of a year\'s month name header', async t => {
    ...
    t.selectorNotExists('.b-cal-event-wrap.b-allday.b-past-event.b-continues-future:contains(Hackathon 2019)', 'Month view not shown');
    ...
    t.selectorExists('.b-cal-event-wrap.b-allday.b-past-event.b-continues-future:contains(Hackathon 2019)', 'Month view shown');
  ...
  t.xit('overflowing events bug 1', async t => {
    eventStore = t.getEventStore({
      data: [{
        ...
        exceptionDates: ['2020-10-13T00:00:00+02:00', '2020-10-22T00:00:00+02:00'],
        resourceId: 'hotel',
        allDay: false
      }, {
        id: 6,
        ...
        exceptionDates: ['2020-10-13T00:00:00+02:00'],
        resourceId: 'hotel',
        allDay: false
      }]
    });
  ...
  t.it('sorting overflowPopup', async t => {
    ...
    t.isDeeply(Array.from(month.overflowPopup.eventList.element.childNodes).map(n => n.innerText.trim()), ['Foxtrot', 'Delta', 'Charlie', 'Bravo', 'Alpha']);
  });
