...
StartTest(t => {
  ...
  function getAllDayHeights() {
    var _resourceView;
    const allDayRows = (_resourceView = resourceView) === null || _resourceView === void 0 ? void 0 : _resourceView.items.map(v => v.allDayEvents).filter(v => v);
  ...
  t.it('Should support showing resourceView with list views', async t => {
    ...
    // TODO make event list respect hideNonWorkingDays and hide events
  ...
  t.it('ResourceView should sync configs of filtered-in subviews', async t => {
    ...
    t.waitForEvent(clarkKent, 'layoutUpdate');
