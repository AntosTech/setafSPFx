...
StartTest(t => {
  const iconClasses = ['b-fa-circle', 'b-fa-flag', 'b-fa-clock', 'b-fa-cloud', 'b-fa-cog', 'b-fa-diamond'];
  let calendar, dayView, eventStore, resourceStore;
  function getDateHours(date) {
    return DateHelper.as('hours', date.getMilliseconds(), 'ms') + DateHelper.as('hours', date.getSeconds(), 'second') + DateHelper.as('hours', date.getMinutes(), 'minute') + date.getHours();
  }
  ...
  function checkEventSanity(t, dayView) {
    ...
    events.forEach(event => {
      ...
      if (eventEl) {
        t.isApproxPx(eventEl.offsetTop, dayView.hourHeight * (getDateHours(event.startDate) - dayView.dayStartHour), `Event ${event.name} top correct`);

        // dayEndTime truncates height.
        // TODO: Calculate and check correct height when truncated by a dayEndTime
        if (!dayView.dayEndTime) {
          t.isApproxPx(eventEl.offsetHeight, DateHelper.as('hours', event.endDate - event.startDate, 'ms') * dayView.hourHeight, `Event ${event.name} height correct`);
        }
      }
    });
  }
  ...
  t.it('Event desc element shrinkwrap height', async t => {
    calendar = await t.getCalendar({
      events: [{
        ...
        // So use y and g
        name: 'ygygygy ygygygy ygygygy ygygygy',
    ...
    const eventEl = calendar.activeView.getEventElement(calendar.eventStore.first),
  ...
  t.it('Click interaction', async t => {
    ...
    t.chain({
      click: '.b-dayview-day-content .b-calendar-cell[data-date="2019-10-15"]',
      offset: ['50%', dayView.hourHeight * 7.5]
    }, next => {
    ...
    {
      click: () => dayView.dayContainerElement,
      offset: [-10, '99%']
    }, () => {
  ...
  t.it('Midnight to midnight events must be in the all day region', async t => {
    const firstDayOfMonth = DateHelper.clearTime(new Date()),
  ...
  const checkDayLayout = async (t, month, day, minWidth, eventCount) => {
    ...
    await t.waitForSelector('.b-cal-event-wrap');
    const eventEls = dayView.dayContentElement.querySelectorAll('.b-cal-event-wrap'),
    ...
    for (const el of eventEls) {
      ...
      if (box.x < space.x) {
        // for test run perf due to many, many assertions
        t.isGreaterOrEqual(box.x, space.x, `Event ${i} on the ${day}th has invalid x (${box.x}) for container (${space.x})`);
      }
      if (box.y < space.y) {
        t.isGreaterOrEqual(box.y, space.y, `Event ${i} on the ${day}th has invalid y (${box.y}) for container (${space.y})`);
      }
      if (box.right > space.right) {
        t.isLessOrEqual(box.right, space.right, `Event ${i} on the ${day}th has invalid right (${box.right}) for container (${space.right})`);
      }
      if (box.bottom > space.bottom) {
        t.isLessOrEqual(box.bottom, space.bottom, `Event ${i} on the ${day}th has invalid bottom (${box.bottom}) for container (${space.bottom})`);
      }
  ...
  t.it('coreHours', async t => {
    ...
    const v = calendar.activeView,
      ...
      expectedBackgroundImage = BrowserHelper.isSafari ? 'linear-gradient(rgba(135, 135, 135, 0.1) 0%, rgba(135, 135, 135, 0.1) 37.5%, transparent 37.5%, transparent 50%, rgba(135, 135, 135, 0.1) 50%, rgba(135, 135, 135, 0.1) 58.33%, transparent 58.33%, transparent 70.83%, rgba(135, 135, 135, 0.1) 70.83%, rgba(135, 135, 135, 0.1) 100%)' : 'linear-gradient(rgba(135, 135, 135, 0.1) 0%, rgba(135, 135, 135, 0.1) 37.5%, rgba(0, 0, 0, 0) 37.5%, rgba(0, 0, 0, 0) 50%, rgba(135, 135, 135, 0.1) 50%, rgba(135, 135, 135, 0.1) 58.33%, rgba(0, 0, 0, 0) 58.33%, rgba(0, 0, 0, 0) 70.83%, rgba(135, 135, 135, 0.1) 70.83%, rgba(135, 135, 135, 0.1) 100%)';
  ...
  t.it('Should fire all event mouse over/out events', async t => {
    ...
    const expectedEvents = ['Mouseover Event 1', 'Mouseout Event 1', 'Mouseover Event 2', 'Mouseout Event 2', 'Mouseover Event 3', 'Mouseout Event 3', 'Mouseover Event 4', 'Mouseout Event 4', 'Mouseover Event 5', 'Mouseout Event 5', 'Mouseover Event 6', 'Mouseout Event 6', 'Mouseover Event 7', 'Mouseout Event 7', 'Mouseover Event 8', 'Mouseout Event 8'],
      firedEvents = [];
    calendar.on({
      eventMouseOver({
        eventRecord
  ...
  t.it('Should show hour subticks when zooming in', async t => {
    ...
    function getVisibleMinorTicks() {
      return t.query('.b-dayview-five-minute-tick').filter(e => DomHelper.getStyleValue(e, 'opacity') == 1);
    }
    t.is(getVisibleMinorTicks().length, 0);
    activeView.hourHeight = activeView.hourHeightBreakpoints.thirty;

    // The half hour ticks are visible
    await t.waitFor(() => getVisibleMinorTicks().length === 24 * 1);
    activeView.hourHeight = activeView.hourHeightBreakpoints.fifteen;

    // The fifteen minute ticks are visible
    await t.waitFor(() => getVisibleMinorTicks().length === 24 * 3);
    activeView.hourHeight = activeView.hourHeightBreakpoints.ten;

    // The ten minute ticks are visible
    await t.waitFor(() => getVisibleMinorTicks().length === 24 * 5);
    activeView.hourHeight = activeView.hourHeightBreakpoints.five;
  ...
  t.it('Zero length events', async t => {
    ...
    t.waitFor(() => calendar.features.eventTooltip._tooltip.isVisible);
  ...
  t.it('default current time indicator', async t => {
    ...
    t.waitForSelector(`.b-dayview-day-detail[data-date="${DateHelper.makeKey(new Date())}"] .b-current-time-indicator:contains()`);
  });
  t.it('currentTimeIndicatorRenderer as a function', async t => {
    ...
    t.waitForSelector(`.b-dayview-day-detail[data-date="${DateHelper.makeKey(new Date())}"] .my-today-indicator:contains(NOW)`);
  });
