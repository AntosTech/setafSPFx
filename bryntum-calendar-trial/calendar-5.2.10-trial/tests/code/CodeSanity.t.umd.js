...
StartTest(t => {
  ...
  t.it('Check $name() and type() methods for Widget classes', async t => {
    const singletonWidgets = ['MessageDialog', 'Ripple'];
    Object.entries(classes).forEach(([className, cls]) => {
      var _cls$prototype;
      // Skip checking singletons
      if (!singletonWidgets.includes(className) && (cls.prototype instanceof classes.Widget || (_cls$prototype = cls.prototype) !== null && _cls$prototype !== void 0 && _cls$prototype.isPrototypeOf(classes.Widget))) {
  ...
  t.it('Check guides links in code are valid', async t => {
    let count = 0;
    const missingGuides = [];
    Object.entries(classes).forEach(([className, cls]) => {
