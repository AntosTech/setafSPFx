...
StartTest(async t => {
  ...
  t.xit('Should have correct SEO links', async t => {
    navigationTree.store.traverse(classRecord => {
      ...
      if (classRecord.id.includes('readme')) {
        //Readme is now the first item in the menu and selected by default, no mask will appear
        return;
      }
      t.it(`Checking ${classRecord.id}`, async t => {
        const link = `#${classRecord.id}`,
