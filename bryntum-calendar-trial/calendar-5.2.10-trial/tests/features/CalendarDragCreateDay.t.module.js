import { Duration, EventStore, ResourceStore } from '../../build/calendar.module.js';
...
StartTest(t => {
    ...
    t.describe('All Day', t => {
        ...
        t.it('should abort drag operation on ESC keypress', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14), new Date(2019, 9, 15), 'New event']
            ]);
        ...
        // https://github.com/bryntum/support/issues/4895
        t.it('Should create event with disabled resize observer', async t => {
            ...
            t.ok(harness.newEventRecord, 'Event created');
    ...
    t.describe('Day Detail', t => {
        ...
        t.it('Should do nothing if disabled', async t => {
            ...
            await harness.dayTimeDrag({
                from : { date : '2019-10-14', time : '2:00' },
        ...
        t.it('Should abort the edit/create if the created event is dragged', async t => {
            ...
            await t.waitFor(() => harness.calendar.features.eventEdit.editor?.containsFocus);
            ...
            await t.waitFor(() => !harness.calendar.features.eventEdit.editor?.isVisible);
        ...
        t.it('should abort drag operation on ESC keypress', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 4), new Date(2019, 9, 14, 5), '4 AMNew event5 AM']
            ]);
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 2), new Date(2019, 9, 14, 5), '2 AMNew event5 AM']
            ]);
        ...
        t.it('Should be able to drag up 3 hours', async t => {
            ...
            await harness.dayTimeDrag({
                from : { date : '2019-10-14', time : '5:00' },
        ...
        t.it('Should not try to sync a new event while it is being created', async t => {
            ...
            t.is(calendar.crudManager.changes, null, 'No persistable changes');
        ...
        t.it('Should scroll the view when dragging close to the edge', async t => {
            ...
            let [,,, startDateX, startDateY] = await harness.dayXY('2021-06-02', '04:00');
            ...
            const { scrollTop } = overflowElement;
    ...
    t.it('Should honour increment setting', async t => {
        calendar = await t.getCalendar({
            date   : new Date(2019, 9, 14),
            events : [],
            modes  : {
        ...
        await t.dragBy({
            source   : '.b-dayview-day-detail.b-calendar-cell[data-date="2019-10-14"]',
            offset   : ['50%', dayView.hourHeight * 11],
            delta    : [0, dayView.hourHeight / 6],
            dragOnly : true
        });

        // Dragging by the increment pixel height should create an event increment length
        t.ok(calendar.features.drag.eventRecord.fullDuration.isEqual(new Duration('10 min')));
    });
});
