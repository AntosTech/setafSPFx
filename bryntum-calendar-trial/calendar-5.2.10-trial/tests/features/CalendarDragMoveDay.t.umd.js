...
StartTest(t => {
  ...
  t.describe('Day Detail', t => {
    ...
    t.it('Should do nothing if validator vetoes', async t => {
      ...
      const event = h.eventStore.getById(h.Event.CheckInHotel),
    ...
    t.it('should abort drag operation on ESC keypress', async t => {
      ...
      t.expect(tentativeDates).toEqual([[new Date(2019, 9, 14, 15), new Date(2019, 9, 14, 19), `3 PM${h.Text.CheckInHotel}7 PM`]]);
      h.calendar.features.drag.footer = null;
      await t.moveCursorBy([0, -170]);
      tentativeDates = h.getTentativeDates();
      t.expect(tentativeDates).toEqual([[new Date(2019, 9, 14, 11), new Date(2019, 9, 14, 15), `11 AM${h.Text.CheckInHotel}`]]);
    ...
    t.it('Should move up 1 hour', async t => {
      ...
      t.ok(t.isDragging(), 'Drag is active');
      tentativeDates = h.getTentativeDates();
      t.expect(tentativeDates).toEqual([[new Date(2019, 9, 14, 13), new Date(2019, 9, 14, 17), `1 PM${h.Text.CheckInHotel}5 PM`]]);
      await h.drop();
    });
    ...
    t.it('should drag an event to midnight and not make it an inter-day or allDay event', async t => {
      ...
      t.ok(t.isDragging(), 'Drag is active');
      tentativeDates = h.getTentativeDates();
      t.expect(tentativeDates).toEqual([[new Date(2019, 9, 14, 20), new Date(2019, 9, 15), `8 PM${h.Text.CheckInHotel}12 AM`]]);
      await h.drop();
      ...
      await h.waitForSelector('.b-dayview .b-dayview-day-container .b-cal-event-wrap:contains(Check-In in Hotel)', {
    ...
    t.it('Should be able to convert short event to an allDay by dragging it to span midnight', async t => {
      ...
      t.ok(t.isDragging(), 'Drag is active');
      tentativeDates = h.getTentativeDates();
      t.expect(tentativeDates).toEqual([[new Date(2019, 9, 14, 21), new Date(2019, 9, 15), `9 PM${h.Text.CheckInHotel}1 AM`]]);
      await h.drop();
      await h.waitForSelector('.b-dayview .b-calendarrow .b-cal-event-wrap:contains(Check-In in Hotel)', 'Correctly dragged into all day zone');
    });
