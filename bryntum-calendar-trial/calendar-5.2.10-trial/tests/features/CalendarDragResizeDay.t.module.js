import { DateHelper, MessageDialog } from '../../build/calendar.module.js';
    ...
    t.describe('Day Detail', t => {
        ...
        t.it('Should be able to drag startDate up and down by 2 hours', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 16), new Date(2019, 9, 14, 18), `4 PM${h.Text.CheckInHotel}6 PM`]
            ]);
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 12), new Date(2019, 9, 14, 18), `12 PM${h.Text.CheckInHotel}6 PM`]
            ]);
        ...
        t.it('Should limit drag startDate to endDate', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 17), new Date(2019, 9, 14, 18), `5 PM${h.Text.CheckInHotel}6 PM`]
            ]);

            // Important to stay inside the drag proxy for Siesta reasons
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 17, 45), new Date(2019, 9, 14, 18), `5:45 PM${h.Text.CheckInHotel}6 PM`]
            ]);
        ...
        t.it('Should be able to drag endDate down and up', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 14), new Date(2019, 9, 14, 16), `2 PM${h.Text.CheckInHotel}4 PM`]
            ]);
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 14), new Date(2019, 9, 14, 19), `2 PM${h.Text.CheckInHotel}7 PM`]
            ]);
        ...
        t.it('should abort drag operation on ESC keypress', async t => {
            ...
            t.expect(tentativeDates).toEqual([
                [new Date(2019, 9, 14, 16), new Date(2019, 9, 14, 18)]
            ]);
        ...
        t.it('Should scroll the view when dragging close to the edge', async t => {
            await h.init(t, {
                ...
                events : [
                    { id : 1, startDate : '2021-06-02T06:00:00', endDate : '2021-06-02T10:00:00', name : 'Event 1' }
                ],
                modes : {
                    day : {
                        visibleStartTime : 0
        ...
        t.it('Should support async finalizing with async beforeDragResizeEnd listener', async t => {
            ...
            h.calendar.on({
                beforeDragResizeEnd : async({ context }) => {
