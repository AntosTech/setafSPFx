...
StartTest(t => {
  let calendar, eventStore, resourceStore, agenda;
  ...
  t.it('Should be able to promote a singular event to be a recurring base', async t => {
    ...
    t.chain({
      waitFor: () => document.querySelectorAll('.b-cal-event').length === 1,
      desc: 'Only one event rendered'
    }, {
      dblclick: '[data-date="2019-10-15"] .b-cal-event:Contains(Recurring Meeting)'
    }, {
  ...
  t.it('Month cell should lay out events correctly when editor aligned to it', async t => {
    ...
    // Collect the event element for measuring
    {
      ...
      const editorTarget = calendar.features.eventEdit.editor.lastAlignSpec.target,
        eventEl = els[0];
  ...
  t.it('allowOverlap should be true by default and allow events to overlap', async t => {
    ...
    const gridIntro = calendar.eventStore.find(e => e.name === 'Scheduler Grid introduction + review'),
      scrum = calendar.eventStore.find(e => e.name === 'Team Scrum');
    t.chain({
      dblclick: '.b-cal-event-wrap:contains(Scheduler Grid introduction + review)',
      offset: ['50%', '20%']
    }, {
  ...
  t.it('Drag-creating an interDay event in WeekView', async t => {
    ...
    t.chain(next => {
      ...
      waitFor: () => {
        var _editor, _editor2, _editor3, _editor4;
        editor = calendar.features.eventEdit.editor;
        recurrenceCombo = (_editor = editor) === null || _editor === void 0 ? void 0 : _editor.widgetMap.recurrenceCombo;
        editRecurrenceButton = (_editor2 = editor) === null || _editor2 === void 0 ? void 0 : _editor2.widgetMap.editRecurrenceButton;

        // Editor contains focus and is aligned to the new event's element.
        return ((_editor3 = editor) === null || _editor3 === void 0 ? void 0 : _editor3.containsFocus) && ((_editor4 = editor) === null || _editor4 === void 0 ? void 0 : _editor4.lastAlignSpec.target) === week.allDayEvents.getEventElement(newRecord);
      }
    }, {
      type: 'Test Event'
    }, {
      ...
      t.selectorCountIs('.b-cal-event-wrap.b-allday:not(.b-overflow)', 2, 'Occurrences appear correctly');
    });
  });
  ...
  t.it('Event preserve currentTimeIndicator when doing autoCreate in current day', async t => {
    ...
    t.chain({
      ...
      dblclick: `[data-date="${DateHelper.format(today, 'YYYY-MM-DD')}"]`,
      offset: [20, 300]
    }, {
      waitFor: () => eventEdit.editor && eventEdit.editor.containsFocus
    }, () => {
  ...
  t.it('Creating new events in all day row should work when allday row is overflowed', async t => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      await t.waitFor(() => {
        ...
        return (_calendar$features$ev2 = calendar.features.eventEdit.editor) === null || _calendar$features$ev2 === void 0 ? void 0 : _calendar$features$ev2.widgetMap.nameField.containsFocus;
      });
  ...
  t.it('Should support vetoing an edit', async t => {
    ...
    await calendar.editEvent(calendar.eventStore.first);
  ...
  t.it('Should edit overflowing event in month view', async t => {
    ...
    await calendar.editEvent(calendar.eventStore.getById(8));
    await t.waitForSelector('.b-eventeditor');
    ...
    t.mockUrl('test-sync', () => {
      return {
        responseText: JSON.stringify({
          ...
          events: {
            rows: [{
              $PhantomId: calendar.eventStore.first.id,
              id: 1,
              name: 'foo',
              startDate: '2019-06-09T12:00:00',
              endDate: '2019-06-09T13:00:00'
            }]
          }
        })
      };
  ...
  t.it('Stores should report added records correctly', async t => {
    ...
    async function createCalendarAndEvent(t, config) {
      var _calendar2;
      (_calendar2 = calendar) === null || _calendar2 === void 0 ? void 0 : _calendar2.destroy();
    ...
    t.it('Using CrudManager (single assignment)', async t => {
      ...
      t.is((_changes$assignments4 = changes.assignments) === null || _changes$assignments4 === void 0 ? void 0 : _changes$assignments4.added[0].resourceId, 1, 'Assignment resource id is ok');
    });
    t.it('Using CrudManager (multi assignment)', async t => {
      var _changes$assignments5, _changes$assignments6, _changes$assignments7, _changes$assignments8;
      ...
      const {
        ...
        assignment = assignmentStore.last,
        event = eventStore.last;
      t.is((_changes$assignments5 = changes.assignments) === null || _changes$assignments5 === void 0 ? void 0 : _changes$assignments5.added.length, 1, 'Assignment is in changeset');
      t.is((_changes$assignments6 = changes.assignments) === null || _changes$assignments6 === void 0 ? void 0 : _changes$assignments6.added[0].$PhantomId, assignment.id, 'Assignment id is ok');
      t.is((_changes$assignments7 = changes.assignments) === null || _changes$assignments7 === void 0 ? void 0 : _changes$assignments7.added[0].eventId, event.id, 'Assignment event id is ok');
      t.is((_changes$assignments8 = changes.assignments) === null || _changes$assignments8 === void 0 ? void 0 : _changes$assignments8.added[0].resourceId, 1, 'Assignment resource id is ok');
    });
  });
  ...
  // https://github.com/bryntum/support/issues/4781
  t.it('Should be possible to edit an event to end at midnight', async t => {
    calendar = await t.getCalendar({
      events: [{
        id: 'test-event',
        name: 'Event from 9am to 3pm',
        startDate: '2022-08-11T09:00',
        endDate: '2022-08-11T15:00'
      }],
      features: {
        eventEdit: true
      },
    ...
    const eventRecord = calendar.eventStore.first,
      {
        eventEdit
      } = calendar.features;
    await t.doubleClick('.b-cal-event-wrap[data-event-id=test-event]');

    // Event editor shown is pass condition. Bug was a JS error thrown.
    await t.waitFor(() => {
      var _eventEdit$_editor;
      return (_eventEdit$_editor = eventEdit._editor) === null || _eventEdit$_editor === void 0 ? void 0 : _eventEdit$_editor.containsFocus;
    });
    await t.type({
      el: eventEdit.editor.widgetMap.nameField.input,
      text: 'Event from 9am to midnight',
      clearExisting: true
    });
    await t.click(eventEdit.editor.widgetMap.endDateField.input);
    await t.type({
      el: eventEdit.editor.widgetMap.endDateField.input,
      text: '08/12/2022',
      clearExisting: true
    });
    await t.click(eventEdit.editor.widgetMap.endTimeField.input);
    await t.type({
      el: eventEdit.editor.widgetMap.endTimeField.input,
      text: '[BACKSPACE][BACKSPACE]AM'
    });
    await t.type({
      el: eventEdit.editor.widgetMap.endTimeField.input,
      text: '[LEFT][LEFT][LEFT][LEFT][LEFT][LEFT][BACKSPACE]12'
    });
    await t.click(eventEdit.editor.widgetMap.saveButton.element);

    // Wait for update
    await t.waitForSelector('.b-cal-event-wrap[data-event-id=test-event]:contains(Event from 9am to midnight)');

    // Event ends at midnight which is 00:00 of the next day
    t.is(eventRecord.endDate, new Date(2022, 7, 12));
    t.is(calendar.getElementFromEventRecord(eventRecord).style.height, '62.5%');
  });

  // https://github.com/bryntum/support/issues/5370
  t.it('Should be possible augment the filters of the resourceField', async t => {
    calendar = await t.getCalendar({
      events: t.getHackathonData().events.rows,
      resources: t.getHackathonData().resources.rows,
      features: {
            ...
            items: {
              ...
              resourceField: {
                ...
                store: {
                  filters: [{
                    filterBy(rec) {
                      return rec.id === 'bryntum' || rec.id === 'hotel';
                    }
                  }]
                }
              }
    ...
    await calendar.editEvent(calendar.eventStore.first);
    await t.click('[data-ref="resourceField"] input');
    const {
        resourceField
    ...
    // We have the custom filter in addition to the primary filter
    t.is(resourceField.store.filters.count, 2);
    await t.type(null, 'b', null, null, null, true);

    // Filtering has worked
    t.isDeeply(store.map(r => r.id), ['bryntum']);
    await t.type(null, 'h', null, null, null, true);

    // Filtering has worked
    t.isDeeply(store.map(r => r.id), ['hotel']);
    await t.type(null, '[ESCAPE]', null, null, null, true);
    await t.click(resourceField.triggers.expand.element);

    // Primary filter is disabled, but the field's store's configured filter is in force.
    // Resource id 'mats' is still filtered out.
    t.isDeeply(store.map(r => r.id), ['bryntum', 'hotel']);
  });
  t.it('Should open all combos, datefields and timefields in all themes and all locales', async t => {
    ...
    await t.forEveryThemeAndLocale(async () => {
      await calendar.editEvent(calendar.events[0]);
      for (const trigger of t.query(`.b-pickerfield .b-icon-picker, .b-pickerfield .b-icon-calendar`)) {
        // Open dropdown list
        await t.click(trigger);
        // Close dropdown list
        await t.click(trigger);
      }
      await t.click('.b-eventeditor .b-popup-close');
    });
  });
});