...
StartTest(t => {
  ...
  t.it('Should be able to drag startDate', async t => {
    ...
    // The 768px height only allows 1 event per row when there is overflow, so work around that:
    ...
    // https://github.com/bryntum/support/issues/4864
    // Focus should be forced into the event start element.
    t.is(document.activeElement, h.calendar.activeView.getEventElement(eventRecord));
    tentativeDates = h.getTentativeDates();
    t.expect(tentativeDates).toEqual([]);
  });
  t.it('Should be able to drag startDate RTL', async t => {
    await h.init(t, {
      rtl: true
    });
    const eventRecord = h.eventStore.getById(h.Event.BeerTime);
  ...
  t.it('Should be able to drag startDate back and forward RTL', async t => {
    await h.init(t, {
      rtl: true
    });
    const eventRecord = h.eventStore.getById(h.Event.BeerTime);
  ...
  t.it('Should be able to drag startDate up into prior week RTL', async t => {
    await h.init(t, {
      rtl: true
    });
    const eventRecord = h.eventStore.getById(h.Event.BeerTime);
  ...
  t.it('Should drag endDate RTL', async t => {
    await h.init(t, {
      rtl: true
    });
    const eventRecord = h.eventStore.getById(h.Event.BeerTime);
  ...
  t.it('Should drag endDate down into future weeks RTL', async t => {
    await h.init(t, {
      rtl: true
    });
    const eventRecord = h.eventStore.getById(h.Event.BeerTime);
  ...
  t.it('Should scroll the view when dragging close to the edge', async t => {
    ...
    let [source, sourceOffset] = await h.hoverRight(1);
    ...
    await t.mouseUp();
    t.is(event1.endDate, new Date(2021, 6, 8), 'End date is ok');
