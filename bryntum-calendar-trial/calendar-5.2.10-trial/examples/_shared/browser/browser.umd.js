...
var groupPaths = {
  'Integration/Angular': 'frameworks/angular',
  'Integration/Ionic': 'frameworks/ionic',
  'Integration/React': 'frameworks/react',
  'Integration/Vue': 'frameworks/vue-3'
};
class ExamplesApp {
  constructor() {
    var me = this;
    ...
    var version = VersionHelper.getVersion(me.product.name),
      ...
      loadFromStorage = name => {
        try {
          sessionStorage.getItem(storageName(name));
        } catch (e) {}
      },
  ...
  getDomConfig() {
    ...
    me.examplesStore.records.forEach(example => {
      if (example.isSpecialRow) {
        ...
        var html = group;
        if (groupPaths[group]) {
          var tip = me.isOnline ? 'Path in distribution after download' : 'Click to view files in the folder if local web server is configured to allow directory listing',
        ...
        var hasPopup = example.build && !me.isOnline || example.offline,
          id = example.id,
          url = me.fixRTL(example.fullFolder);
        configs.push({
          tag: 'a',
          className: {
            example: 1,
          ...
          id,
          draggable: false,
          href: !example.offline ? url : undefined,
          dataset: {
  ...
  render() {
    ...
    Widget.attachTooltip(me.examplesContainerEl, {
      ...
      getHtml: async ({
        ...
        var url = `${linkNode.getAttribute('href') || linkNode.dataset.linkUrl}/app.config.json`;
    ...
    EventHelper.on({
      ...
      click(event) {
        var el = event.target.closest('[data-link-url]');
        new Popup({
          forElement: el,
          maxWidth: '18em',
