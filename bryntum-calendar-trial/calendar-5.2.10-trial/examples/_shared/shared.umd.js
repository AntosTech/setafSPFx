...
window.bryntum.product = {
  ...
  appId: '56c4eb365f023b3b2c6d65623914ca87db182c4a'
};
/* global RC */
// Load product config
...
class Shared extends Localizable(Events()) {
  ...
  insertHeader() {
    ...
    header.innerHTML = `
            <div id="title-container">
                <a id="title" href="${isDemoBrowser ? '#' : '../'}${!isDemoBrowser && isUmd ? 'index.umd.html' : ''}${this.rtl ? '?rtl' : ''}#example-${exampleId}">
                    <h1>${exampleName}</h1>
                </a>
            </div>
            <div id="tools"></div>
  ...
  async initHints() {
    ...
    var targetsAreReady = true;

    // check if all hint targets are available
    for (var key of Object.keys(hints)) {
      var target = key && DomHelper.down(document.body, key);
      if (!target) {
        targetsAreReady = false;
        break;
      }
    }
    ...
    for (var [_key, hint] of Object.entries(hints)) {
      var _target = _key && DomHelper.down(document.body, _key);
      if (_target) {
  ...
  loadDescription() {
    ...
    AjaxHelper.get(url, {
      ...
      var localeValue = me.localeManager.locale.localeName,
        themeCombo;
      var storedLocaleValue = BrowserHelper.getLocalStorageItem('b-example-language');
  ...
  async updateJS() {
    ...
    try {
      ...
      DomHelper.forEachSelector(document.body, '.b-widget', element => {
        var widget = Widget.fromElement(element);
...
_defineProperty(CodeEditor, "configurable", {
  textContent: false,
  scrollable: null,
  collapsible: {
    type: 'overlay',
  ...
  collapsed: true,
  monitorResize: false,
  title: CodeEditor.L('<i class="b-icon b-icon-code"></i> L{Code editor}'),
  codeCache: {},
  tools: {
    download: {
